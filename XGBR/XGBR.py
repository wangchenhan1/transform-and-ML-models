...
# 从Excel文件中读取数据
data = pd.read_excel('/home/ggx/wls/DAC-code/HCOOH-2025.10.31/HCOOH-O第一轮/data/DAC-HCOOH-O-1.xlsx')
...
# 定义超参数范围（针对 XGBRegressor）
param_grid = {
    "n_estimators": list(range(50, 750, 50)),
...
for state in random_states:
    ...
    # 定义XGBRegressor模型
    XGBR = XGBRegressor(random_state=state)  # 使用当前state作为random_state

    # 使用RandomizedSearchCV来寻找最优超参数
    grid_search = RandomizedSearchCV(
        XGBR, param_grid, n_iter=50, cv=5, n_jobs=-1, scoring='r2'
    )
...
# =============================================================================
print("\n" + "="*80)
print("🎯 BEST MODEL PARAMETERS AND PERFORMANCE SUMMARY")
print("="*80)
...
# 保存详细结果到文件
output_file = os.path.join(current_directory, "best_model_summary.txt")
with open(output_file, 'w') as f:
    f.write("BEST XGBOOST MODEL SUMMARY\n")
    f.write("="*50 + "\n\n")
...
plt.title(f"XGBoost Regression - Best Model (Random State: {best_random_state})", fontsize=14, fontweight='bold')
...
# 保存散点图
scatter_plot_filename = os.path.join(current_directory, "best_model_scatter_plot.png")
...
plt.xlabel("Train R²")
plt.ylabel("Test R²")
plt.title("Train R² vs Test R² for All Random States")
plt.grid(True, alpha=0.3)
...
# 保存训练vs测试R²图
r2_comparison_filename = os.path.join(current_directory, "train_vs_test_r2_comparison.png")
...
try:
    # 使用部分数据以避免内存问题
    if len(X_test_normalized_correct) > 100:
        X_test_sample = X_test_normalized_correct[:100]
    else:
        X_test_sample = X_test_normalized_correct
    ...
    # 保存 SHAP 图
    shap_plot_filename = os.path.join(current_directory, "best_model_shap_analysis.png")
    ...
    feature_plot_filename = os.path.join(current_directory, "best_model_feature_importance.png")
...
results_df = pd.DataFrame({
    'random_state': list(results_all_states.keys()),
    'train_r2': [result['train_r2'] for result in results_all_states.values()],
    'test_r2': [result['test_r2'] for result in results_all_states.values()],
    'train_mae': [result['train_mae'] for result in results_all_states.values()],
    'test_mae': [result['test_mae'] for result in results_all_states.values()],
    'train_rmse': [result['train_rmse'] for result in results_all_states.values()],
    'test_rmse': [result['test_rmse'] for result in results_all_states.values()]
})

results_csv_filename = os.path.join(current_directory, "all_random_states_results.csv")
results_df.to_csv(results_csv_filename, index=False)
print(f"📊 All random states results saved as: {results_csv_filename}")

# 输出统计摘要
print(f"\n📈 RESULTS STATISTICAL SUMMARY:")
print("-" * 50)
print(f"Train R² - Mean: {results_df['train_r2'].mean():.4f}, Std: {results_df['train_r2'].std():.4f}")
print(f"Test R²  - Mean: {results_df['test_r2'].mean():.4f}, Std: {results_df['test_r2'].std():.4f}")
print(f"Train MAE - Mean: {results_df['train_mae'].mean():.4f}, Std: {results_df['train_mae'].std():.4f}")
print(f"Test MAE  - Mean: {results_df['test_mae'].mean():.4f}, Std: {results_df['test_mae'].std():.4f}")