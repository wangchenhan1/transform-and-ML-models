...
# 定义超参数范围（针对 AdaBoostRegressor）
param_grid = {
    "n_estimators":list(range(50, 1000, 50)),
    "learning_rate": [0.06, 0.01, 0.2, 0.5, 0.8],
    "estimator__max_depth": list(range(2, 10, 1)),
    "estimator__min_samples_leaf": [1, 2, 5, 7, 9],
    "estimator__max_features": [1, 2, 3, 4, 6, 8, 12],
    "estimator__min_samples_split": [1, 2, 4, 6, 8, 10, 12],
    "estimator__min_weight_fraction_leaf": [0, 0.02, 0.04, 0.06, 0.08],
    "estimator__min_impurity_decrease": [0.008, 0.01]
}
...
for state in random_states:
    ...
    grid_search = RandomizedSearchCV(
        ABR, param_grid, n_iter=50, cv=5, n_jobs=-1, scoring='r2'
    )
...
best_model_filename = os.path.join(current_directory, "best_adaboost_model.pkl")
joblib.dump(best_model, best_model_filename)

best_scaler_filename = os.path.join(current_directory, "best_adaboost_scaler.pkl")
...
# =============================================================================
print("\n" + "="*80)
print("🎯 BEST ADABOOST MODEL PARAMETERS AND PERFORMANCE SUMMARY")
print("="*80)
...
# 保存详细结果到文件
output_file = os.path.join(current_directory, "best_adaboost_model_summary.txt")
with open(output_file, 'w') as f:
    f.write("BEST ADABOOST REGRESSOR MODEL SUMMARY\n")
    f.write("="*50 + "\n\n")
    ...
    f.write(f"Base Estimator: DecisionTreeRegressor\n\n")
...
plt.title(f"AdaBoost Regression - Best Model (Random State: {best_random_state})", fontsize=14, fontweight='bold')
...
# 保存散点图
scatter_plot_filename = os.path.join(current_directory, "best_adaboost_model_scatter_plot.png")
...
plt.xlabel("Train R²")
plt.ylabel("Test R²")
plt.title("Train R² vs Test R² for All Random States (AdaBoost)")
plt.grid(True, alpha=0.3)
...
# 保存训练vs测试R²图
r2_comparison_filename = os.path.join(current_directory, "adaboost_train_vs_test_r2_comparison.png")
...
try:
    ...
    plt.title("SHAP Feature Importance (AdaBoost)", fontsize=14, fontweight='bold')
    plt.tight_layout()

    # 保存 SHAP 图
    shap_plot_filename = os.path.join(current_directory, "best_adaboost_model_shap_analysis.png")
    ...
    feature_plot_filename = os.path.join(current_directory, "best_adaboost_model_feature_importance.png")
...
print("\n" + "="*80)
print("🎉 ALL ADABOOST ANALYSES COMPLETED SUCCESSFULLY!")
print("="*80)